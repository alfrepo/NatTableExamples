package de.example.nebula.nattable.progress;
...
import de.example.nebula.nattable.comboboxdropdown.PercentageConverter;
import de.example.nebula.nattable.comboboxdropdown.TextCellRenderer;
...
import de.example.nebula.nattable.widget.progress.IPercentageConverter;
import de.example.nebula.nattable.widget.progress.PercentageMBarDecorator;
...
public class Table extends AbstractNatExample {
	...
	private IDataProvider setupBodyDataProvider() {
		final List people = Arrays.asList(
				new MyPerson(100, "Mickey Mouse", new Date(1000000), true, 0), 
				new MyPerson(110, "Batman", new Date(2000000), false, 5), 
				new MyPerson(110, "Batman", new Date(2000000), false, 10), 
				new MyPerson(110, "Batman", new Date(2000000), false, 20), 
				new MyPerson(110, "Batman", new Date(2000000), false, 30), 
				new MyPerson(120, "Bender", new Date(3000000), false, 40), 
				new MyPerson(120, "Bender", new Date(3000000), false, 50), 
				new MyPerson(130, "Cartman", new Date(4000000), false, 60), 
				new MyPerson(130, "Cartman", new Date(4000000), false, 70), 
				new MyPerson(140, "Dogbert", new Date(5000000), true, 90),
				new MyPerson(140, "Dogbert", new Date(5000000), true, 100));
	...
	private void registerCellRenderers(NatTable natTable){
		...
		// renderer
		ICellPainter linkCellRenderer = new LinkCellRenderer();
		// converter
		IPercentageConverter percentageConverter = new IPercentageConverter() {
			@Override
			public int convert(ILayerCell layerCell, IConfigRegistry configRegistry, Object object) {
				Integer integer = (Integer) object;
				return integer;
			}
		};
		PercentageMBarDecorator percentageMBarDecorator = new PercentageMBarDecorator(linkCellRenderer, percentageConverter);
		configRegistry.registerConfigAttribute(CellConfigAttributes.CELL_PAINTER, percentageMBarDecorator, DisplayMode.NORMAL, "LABEL4");
	}
...
//		configRegistry.registerConfigAttribute(EditConfigAttributes.CELL_EDITOR, new DoubleValidatedEditor(), DisplayMode.EDIT, "LABEL0");
	...
	public class MyPerson extends Person{
		...
		private MyPerson(int id, String name, Date birthDate, boolean isAnimal, int distance) {
			super(id, name, birthDate);
