...
/** {@inheritDoc}
 * 
 * This ViewPort supports external scrollBars.
 *
 */
public class NattableViewportLayer extends ViewportLayer implements IUniqueIndexLayer {


	protected NattableHorizontalScrollBarHandler hBarListener;
	protected NattableVerticalScrollBarHandler vBarListener;
	...
	List<ListenerRowPosition> rowPositionListeners = new ArrayList<>();
	
	boolean mayModifyHorizontalScrollbar = false;
	boolean mayModifyVerticalScrollbar = false;
	
	/**
	 * 
	 * @param underlyingLayer
	 * @param horizontalScrollBar - will be used to control the viewportLayer. May be null.
	 * @param verticalScrollBar - will be used to control the viewportLayer. May be null.
	 */
	public NattableViewportLayer(IUniqueIndexLayer underlyingLayer, ScrollBar horizontalScrollBar, boolean mayModifyHorizontalScrollbar,  ScrollBar verticalScrollBar, boolean mayModifyVerticalScrollbar) {
		...
		this.mayModifyHorizontalScrollbar = mayModifyHorizontalScrollbar;
		this.mayModifyVerticalScrollbar = mayModifyVerticalScrollbar;
	}
	...
	public void setOriginRowPosition(int scrollableRowPosition) {
		...
		for(ListenerRowPosition l:rowPositionListeners ){
			l.handleSetRowPosition(scrollableRowPosition);
		}
	}
	
	public void addScrollableRowPositionListener(ListenerRowPosition rowPositionListener){
		rowPositionListeners.add(rowPositionListener);
	}
	public void removeScrollableRowPositionListener(ListenerRowPosition rowPositionListener){
		rowPositionListeners.remove(rowPositionListener);
	}
	...
	public boolean doCommand(ILayerCommand command) {
		boolean superResult = super.doCommand(command);
		...
		if (command instanceof ClientAreaResizeCommand && command.convertToTargetLayer(this)) {
			...
			//hide the NatTable's scrollBars
			ScrollBar hBarNatTabelNative = clientAreaResizeCommand.getScrollable().getHorizontalBar();
			ScrollBar vBarNatTabelNative = clientAreaResizeCommand.getScrollable().getVerticalBar();
			
			hBarNatTabelNative.setVisible(false);
			vBarNatTabelNative.setVisible(false);
			...
			if (hBarListener == null && hBar != null) {
				hBarListener = new NattableHorizontalScrollBarHandler(this, hBar, mayModifyHorizontalScrollbar);
			}
			if (vBarListener == null && vBar != null) {
				vBarListener = new NattableVerticalScrollBarHandler(this, vBar, mayModifyVerticalScrollbar);
			}

			//this method deactivates the ScrollBars
			handleGridResize();
